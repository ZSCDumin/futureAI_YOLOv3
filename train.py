...
args.train_img_cnt = len(open(args.train_file, 'rb').readlines())
args.val_img_cnt = len(open(args.val_file, 'rb').readlines())
...
train_dataset = train_dataset.apply(tf.contrib.data.map_and_batch(
    lambda x: tf.py_func(parse_data, [x, args.class_num, args.img_size, args.anchors, 'train'],
                         [tf.float32, tf.float32, tf.float32, tf.float32]),
    num_parallel_batches=args.num_threads, batch_size=args.batch_size))
